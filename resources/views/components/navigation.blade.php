<nav class="bg-gray-800" x-data="{ open: false }">
        ...
        <div class="flex h-16 items-center justify-between">
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <img class="h-8 w-8" src="images/magic-ball.png" alt="Your Company">
                </div>
                    ...
                    <div class="relative ml-3">
                        ...
                        <div class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                            ...
                            @foreach ($rightmenu as $item)
                                ...
                                @else
                                    <a href="{{ $item['url'] }}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem">
                                        {{ $item['name'] }}
                                    </a>
                                @endif
                            @endforeach
    ...
    <div class="md:hidden" id="mobile-menu" x-show="open">
        <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
            @foreach($menu as $item)
            <a href="{{$item['url']}}" class="text-gray-300 hover:bg-gray-700 hover:text-white block rounded-md px-3 py-2 text-base font-medium">{{$item['name']}}</a>
            @endforeach
        </div>
    </div>
</nav>